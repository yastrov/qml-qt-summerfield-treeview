...
void TreeModel::initialize(){
    ...
    m_roleNameMapping[TreeModel_Roles::Role_TODAY] = "today";
    m_roleNameMapping[TreeModel_Roles::Role_TOTAL] = "total";
    m_roleNameMapping[TreeModel_Roles::Role_DONE] = "done";
    #endif
}
...
#ifdef QML_MODEL
    if (!index.isValid()) {
        qDebug() << "TreeModel::flags Item is invalid";
...
#ifdef QML_MODEL
        theFlags |= Qt::ItemIsEditable|Qt::ItemIsUserCheckable|
...
{
#ifdef QML_MODEL
    qDebug()<<"TreeModel::data role: "<<role;
    ...
    if (TaskItem *item = itemForIndex(index)) {
        ...
        case TreeModel_Roles::Role_NAME:
            return item->name();
            break;
        case TreeModel_Roles::Role_TODAY:
            return item->todaysTime();
            break;
        case TreeModel_Roles::Role_TOTAL:
            return item->totalTime();
            break;
        case TreeModel_Roles::Role_DONE:
    ...
    if (!rootItem || !index.isValid() || index.column() < 0 ||
        ...
        return QVariant();
    if (TaskItem *item = itemForIndex(index)) {
        if (role==TreeModel_Roles::Role_TITLE) {
            return item->name();
        }
                        ...
                        const QVariant &value, int role)
{
    ...
    if (TaskItem *item = itemForIndex(index)) {
        switch (role) {
        case TreeModel_Roles::Role_NAME:
            qDebug() << "item->setName(value.toString()) "<<value.toString();
...
QModelIndex TreeModel::cut(const QModelIndex &index)
{
    ...
    child = nullptr; // Silence compiler unused variable warning
